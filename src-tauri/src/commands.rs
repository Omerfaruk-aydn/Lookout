...
use crate::orchestrator::{self, FullAnalysisResult, SentimentResult, VisionResult};
use crate::AppState;
use base64::Engine;
use tauri::State;
...
pub async fn run_analysis(
    ...
    let vision_task: tokio::task::JoinHandle<Result<VisionResult, LookoutError>> =
        if let Some(ref img) = image_base64 {
            ...
            tokio::spawn(async move {
                orchestrator::call_vision_sidecar(&path, &img_clone, "anthropic/claude-sonnet-4-6").await
            })
        } else {
    ...
    let news_task = tokio::spawn(async move {
        orchestrator::call_news_sidecar(&news_path, &news_ticker, 48, "anthropic/claude-haiku-3.5").await
    });
    ...
    let (vision_result, news_result, data_result) =
        tokio::join!(vision_task, news_task, data_task);
